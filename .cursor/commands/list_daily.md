...
1. Read `config/notion.ts`, parse `notionConfig.database.dailyNotes` to get the
   database title.
2. Using the Notion MCP:
   - Search for the database by title and get its database/page ID.
   - Fetch the database details; extract the first `collection://...` data
     source URL.
...
3. For each entry, gather the properties:
   ...
   - Date: use a `Date` property if present; otherwise fall back to created
     time.
...
4. Sort entries by Date desc (fallback: created time desc). Apply `limit`
   (default 7).
...
- If multiple databases match the title, prefer the one under the
  `workspaceName` parent page if present.
