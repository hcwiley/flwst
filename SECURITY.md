# Security Policy

## Reporting a Vulnerability

- Please do not open public issues with exploitable details.
- Preferred: open a GitHub Security Advisory for this repository if available.
- Alternatively, contact the maintainer privately (see repository owner profile)
  with details.
- Provide a minimal reproduction and affected versions if possible.

## Supported Versions

- Current development branch: supported.
- Pre-release versions (0.x): security fixes are best-effort.

## Disclosure

- We will acknowledge receipt within a reasonable time and work on a fix.
- Once a fix is available, we will credit reporters who desire attribution.
